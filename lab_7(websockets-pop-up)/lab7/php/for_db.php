<?
    ...
    if(isset($_POST))
    {
        ...
        {
            mysql_query("DELETE FROM chat WHERE id = '{$_POST['del_id']}'", $link);
        }
...
/* Вывод таблицы из БД */
    ...
    while($result = mysql_fetch_array($sql))#функция вывода таблицы 
    {
        ...
        <td><button type='button' class='moreButton btn btn-primary' onClick='send_id({$result['id']})' data-bs-toggle='modal' data-bs-target='#exampleModal'>Подробнее</button></td>
        </tr>";
    }
